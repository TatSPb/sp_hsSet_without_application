...
public class EmployeeBook {
    ...
    private String getKey(Employee employee) {
        return employee.getName() + " " + employee.getPatronymicName() + " " + employee.getSurname();
    }
    ...
    public void addEmployee(String name, String surname, String patronymicName, double salary, int department) {
        addEmployee(new Employee(name, patronymicName, surname, salary, department));
    }
    ...
    public void changeSalary(Employee employee, double newSalary) {
        ...
        if (employees.containsKey(key)) {
            employees.get(getKey(employee)).setSalary(newSalary);
        }
    }
    ...
    public void changeDepartment(Employee employee, int newDepartment) {
        String key = getKey(employee);
        if (employees.containsKey(key)) {
            employees.get(getKey(employee)).setDepartment(newDepartment);
        }
    }
    ...
    public void printEmployeesByDepartment() {
        ...
        for (int department = 1; department <= 5; department++) {
            System.out.println("Department " + department + " staff: ");
    ...
    public void printEmployeesWithSalaryLessThan(double bound) {
        System.out.println("Staff with salary less than: " + bound + ":");
        for (Employee employee : employees.values()) {
            if (Objects.nonNull(employee) && employee.getSalary() < bound) {
                System.out.printf(
                        "id: %d, %s %s %s, salary: %2f",
                        employee.getId(),
                        employee.getSurname(),
                        employee.getName(),
    ...
    public void printEmployeesWithSalaryGreaterOrEqualThan(double bound) {
        System.out.println("Staff with salary more than or equals to: " + bound + ":");
        for (Employee employee : employees.values()) {
            if (Objects.nonNull(employee) && employee.getSalary() >= bound) {
                System.out.printf(
                        "id: %d, %s %s %s, salary: %2f",
...
//        .orElse(null);
    ...
    public void printAllEmployeesFromDepartment(int department){
        for (Employee employee : employees.values()) {
            if(Objects.nonNull(employee) && employee.getDepartment() == department){
                System.out.printf("id: %d, %s %s %s, salary: %2f, department: %d",
                        employee.getId(),
                        employee.getSurname(),
                        employee.getName(),
                        employee.getPatronymicName(),
